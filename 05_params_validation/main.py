...
@app.get("/user")
async def get_user(
    page_index: int = Query(default=1, title="Page Index", ge=1, le=100)
):
...
async def get_books(
    book_name: str = Path(..., title="book name", min_length=3, max_length=10)
):
...
async def get_items(
    item_no: str = Path(..., title="item No.", regex="^[a-f]-[\\d]*$")
):
