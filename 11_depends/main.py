...
async def verify_auth(
    api_token: Optional[str] = Header(default=None, alias="api-token-header")
):
    if not api_token:
...
@app.get("/items")
async def get_items(
    page_index: Optional[int] = 1, page_size: Optional[int] = 10
):
...
@app.get("/users_depends_func", dependencies=[Depends(verify_auth)])  # 依賴授權
...
class PageInfo:
    def __init__(
        self, page_index: Optional[int] = 1, page_size: Optional[int] = 10
    ):
...
async def get_items_by_class(page_info: PageInfo = Depends(PageInfo)):
    ...
    return {
        "page_index": page_info.page_index,
        "page_limit": page_info.page_size,
    }
